...
> **核心目标**：全链路自动化生成 TTS 配音文案并替换视频音轨，零 API 成本
> **决策日期**：2026-08-10
...
素材视频
  ...
  ├─→ 步骤1: 提取原对白文本 ─────────────────────┐
  │     ├── 有字幕轨？→ ffmpeg 直接抽 .srt        │
  │     └── 没有     → sherpa-onnx ASR（本地离线）  │
  │                                                │
  ├─→ 步骤2: DeepSeek 改写文案 ───────────────────┤
  │     复用 openteam 的浏览器操控技术（0 API Token）│
  │     原对白 → better TTS 配音文案                │
  │                                                │
  ├─→ 步骤3: MiMo TTS 生成新语音（已实现 ✅）       │
  │                                                │
  └─→ 步骤4: ffmpeg 替换音频轨（已实现 ✅）         │
                                                     ▼
                                              最终产物
```
...
| 步骤 | 状态 | 说明 |
|---|---|---|
| ffmpeg 音频轨替换 | ✅ 已实现 | `dedup_video` + TTS 参数 |
| MiMo TTS 语音生成 | ✅ 已实现 | `tts_client.py` |
| DeepSeek 文案改写 | 🔜 待实现 | 复用 openteam 浏览器操控 |
| sherpa-onnx ASR | 🔜 待实现 | 本地离线，ONNX 模型 ~239MB |
| ffmpeg 抽字幕 | 🔜 待实现 | 仅对有字幕轨的素材生效 |
> **日期**：2026-08-10
